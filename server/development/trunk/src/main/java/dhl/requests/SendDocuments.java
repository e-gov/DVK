...
public class SendDocuments {
        ...
        try {
            ...
            org.apache.axis.attachments.AttachmentPart px =
                    (org.apache.axis.attachments.AttachmentPart) context.getCurrentMessage()
                            .getAttachmentsImpl().getAttachmentByReference(bodyData.dokumendid);
            if (px == null) {
            ...
            for (int i = 0; i < docFiles.subFiles.size(); ++i) {
                try {
                    ...
                    if (Settings.Server_RunOnClientDatabase) {
                        ...
                        // saaksime XML parsimisel võtta välja eraldi kirjed, kui sama sõnumit
                        ...
                        Document doc = Document.fromXML(docFiles.subFiles.get(i), user.getOrganizationID(),
                                Settings.Server_ValidateXmlFiles || Settings.Server_ValidateSignatures, conn, xTeePais);
                        ...
                        // Määrame dokumendi säilitustähtajaks vaikimisi väärtuse
                        ...
                        if ((doc.getSendingList() != null) && Settings.Server_DocumentSenderMustMatchXroadHeader) {
                            ...
                            for (int j = 0; j < doc.getSendingList().size(); ++j) {
                                ...
                                if ((tmpSending.getSender() != null)
                                        && (tmpSending.getSender().getOrganizationID() != user.getOrganizationID())
                                        && (tmpSending.getProxy() != null)
                                        && (tmpSending.getProxy().getOrganizationID() != user.getOrganizationID())) {
    ...
    /**
     * Uus versioon päringust. Muudatused:
     * <p/>
    ...
    private static RequestInternalResult generalSendDocuments(
        ...
        try {
            ...
            if ((bodyData.fragmenteKokku <= 0) || (bodyData.fragmentNr == (bodyData.fragmenteKokku - 1))) {
                ...
                FileSplitResult docFiles = CommonMethods.splitOutTags(pipelineDataFile, "DecContainer", true, false, true);
