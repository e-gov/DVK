...
public class SendDocuments {
    ...
    public static RequestInternalResult V1(org.apache.axis.MessageContext context, Connection conn, UserProfile user, OrgSettings hostOrgSettings, XHeader xTeePais) throws Exception, ParserConfigurationException {
        ...
        try {
            // Laeme pï¿½ringu keha endale sobivasse andmestruktuuri
            ...
            // - Kui DVK server on seadistatud tï¿½ï¿½tama kliendi tabelite peal,
            ...
            for (int i = 0; i < docFiles.subFiles.size(); ++i) {
                try {
                    ...
                    // Sï¿½ltuvalt sellest, kas server tï¿½ï¿½tab serveri vï¿½i kliendi
                    // andmebaasi peal, koostame DVK konteineri XML failidest
                    // vastavad andmeobjektid ja salvestame need andmebaasi
                    if (Settings.Server_RunOnClientDatabase) {
                        // Vï¿½tame vï¿½lja antud andmebaasis seadistatud asutuste nimekirja, et
                        // saaksime XML parsimisel vï¿½tta vï¿½lja eraldi kirjed, kui smaa sï¿½numit
                        ...
                        for (int j = 0; j < msgList.size(); ++j) {
                            ...
                            // Kontrollime, et dokumendi saatjaks vï¿½i vahendajaks mï¿½rgitud asutus
                        ...
                        // Vajadusel kontrollime saadetavate .ddoc ja .bdoc failide allkirjad ï¿½le
                        validateSignedFileSignatures(doc.getFiles());
                        
                        // Lisame vï¿½ï¿½rtused sï¿½numi pï¿½isest ja kehast
                        doc.setFolderID( senderTargetFolder );
                        doc.setOrganizationID( user.getOrganizationID() );
                        
                        // Mï¿½ï¿½rame dokumendi sï¿½ilitustï¿½htajaks vaikimisi vï¿½ï¿½rtuse
            ...
            // uute dokumentide ID-de pï¿½hjal vastussï¿½numi.
            try {
                t.reset();
                ...
                if (Settings.Server_RunOnClientDatabase) {
                    ...
                    for (int i = 0; i < clientDocuments.size(); ++i) {
                        ...
                        // Tagastame siin kirje ID asemel DHL_ID vï¿½ï¿½rtuse, kuna
                        // kliendi poolel peaks DVK unikaalne identifikaator
                        // asuma just sellel andmevï¿½ljal.
                        if (tmpMsg.getId() > 0) {
                    ...
                    for (int i = 0; i < serverDocuments.size(); ++i) {
                        ...
                        // Edastame dokumendi vajadusel mï¿½nda teise DVK serverisse
                ...
                // Kustutame ajutisest kataloogist ï¿½ra andmebaasi salvestatud failid
                t.reset();
                if (Settings.Server_RunOnClientDatabase) {
                    for (int i = 0; i < clientDocuments.size(); ++i) {
    ...
    public static RequestInternalResult V2(org.apache.axis.MessageContext context, Connection conn, UserProfile user, OrgSettings hostOrgSettings, XHeader xTeePais) throws Exception, ParserConfigurationException {
        ...
        try {
            ...
            if (bodyData.fragmenteKokku > 0) {
                if (Settings.Server_RunOnClientDatabase) {
                    // Kui DVK server on seadistatud tï¿½ï¿½tama kliendi andmetabelite peal,
                    // siis teostame fragmentide tï¿½ï¿½tlemise andmebaasi asemel kï¿½vakettal
                    ...
                    try {
                        ...
                        if (Settings.Server_RunOnClientDatabase) {
                            ...
                            if (conservationDeadline == null) {
                                // Kui saatja on dokumendi sï¿½ilitustï¿½htaja mï¿½ï¿½ramata jï¿½tnud vï¿½i mï¿½ï¿½ranud
                                // tï¿½htaja vigaselt, siis mï¿½ï¿½rame dokumendi sï¿½ilitustï¿½htajaks vaikimisi vï¿½ï¿½rtuse
                                Calendar calendar = Calendar.getInstance();
                                calendar.setTime(new Date());
                                calendar.add(Calendar.DATE, Settings.Server_DocumentDefaultLifetime);
                                doc.setConservationDeadline(calendar.getTime());
    ...
    /**
     * Uus versioon pï¿½ringust. Muudatused:
     * 
                	...
                	for (int j = 0; j < addressKeys.size(); ++j) {
                        ...
                        if (CommonMethods.copyFile(doc.getFilePath(), simplifiedFile)) {
                            ...
                            if ((server != null) && (server.getAddress() != null) && !server.getAddress().equalsIgnoreCase("")) {
                                ...
                                for (int i = 0; i < orgArray.size(); ++i) {
                                    // Märgime dokumendi ID vastuvõtja andmete juurde
