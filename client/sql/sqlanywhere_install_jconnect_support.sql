...
go
insert dbo.spt_mda values ('SAVEPOINTSUPPORT', 4, '1', 4, 7, 0, -1)
go
...
go
insert dbo.spt_mda values ('GET_ISOLATION', 2, 'select @@isolation', 1, 7, 0, -1)
go
insert dbo.spt_mda values ('SET_ROWCOUNT', 2, 'set rowcount ?', 1, 7, 0, -1)
go
insert dbo.spt_mda values ('GET_AUTOCOMMIT', 2, 'select @@tranchained', 1, 7, 0, -1)
go
...
go
insert dbo.spt_mda values ('SAVEPOINT', 2, 'savepoint', 1, 7, 0, -1)
go
insert dbo.spt_mda values ('ROLL_TO_SAVEPOINT', 2, 'rollback to savepoint', 1, 7, 0, -1)
go
...
go
insert dbo.spt_mda values ('USERNAME', 2, 'select dbo.sp_jconnect_trimit(user_name())', 1, 7, 0, -1)  
go
...
declare owner_cur cursor for 
	...
	while (@@sqlstatus = 0)
        ...
        where SYS.SYSTABLE.table_id = @table_id and SYS.SYSINDEX.index_id = @indexID
	fetch colIDCursor into @colID, @indexID, @sequence, @asc_or_desc
    end
    close colIDCursor 
    fetch owner_cur into @owner
...
BEGIN
   ...
   else if (@datatype = 37 and @usrtype = 36)
   begin
