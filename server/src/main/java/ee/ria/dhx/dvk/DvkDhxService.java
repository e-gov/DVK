...
/**
 * Class is an example implementation of DhxImplementationSpecificService
 * interface. All data that need to be stored is stored in memory.
 * 
...
public class DvkDhxService implements DhxImplementationSpecificService {
	...
	public String receiveDocument(IncomingDhxPackage document,
		...
		try {
			...
			if (StringUtil.isNullOrEmpty(recipientAsutus.getRegistrikood())) {
			 ...
			 //if we are dealing with subsystem, then maybe we need to replace DVK adressee(subsytem name) with DHX(membercode)
            ...
            if (document.getParsedContainer() !=null) {
              ...
              if(container.getDecMetadata() != null && container.getDecMetadata().getDecFolder() !=null) {
                senderTargetFolder = Folder.getFolderIdByPath( container.getDecMetadata().getDecFolder(), user.getOrganizationID(), conn, false, true, null);
              }
            }
	...
	private void saveOrUpdateAsutuss(Connection conn,
		...
		for (InternalXroadMember member : members) {
			...
			if(member.getRepresentee() != null && config.getMemberCode().equals(member.getMemberCode())) {
				selfRepresentees.add(member);
				continue;
			}
			if(member.getMemberCode().equals(config.getMemberCode()) && member.getSubsystemCode().equals(config.getDefaultSubsystem())) {
				selfMember = member;
			}
			Asutus asutus = new Asutus();
		...
		if(needToUpdateSelfRepresentees && selfRepresentees != null && selfRepresentees.size()>0) {
			...
			if(selfMember == null) {
				throw new DhxException(DhxExceptionEnum.TECHNICAL_ERROR, "Unable to find own member in address list to add representees.");
			}
	...
	public String getDhxSubsystem(Asutus dvkAsutus) {
		if (dvkAsutus.getRegistrikood2() == null) {
			...
			/*if(!subsystem.startsWith(config.getDhxSubsystemPrefix())) {
				subsystem = config.getDhxSubsystemPrefix() + "." + subsystem;
			}*/
			return subsystem;
		}
	}
	
	//returns true if given subsystem is special DVK subsystem with which registrikood equals subsystemname, not subsystemname.membercode
		...
		if (member.getRepresentee() != null) {
			if (member.getRepresentee().getRepresenteeSystem() != null) {
				if(isSpecialSubsystem(member.getRepresentee().getRepresenteeSystem())) {
					...
					regCode = member.getRepresentee().getRepresenteeSystem() + "." + member.getRepresentee().getRepresenteeCode();
				}
			} else {
	...
	/**
	 * If the document was already sent, then no need to resend immediately,
	...
	/*private Boolean needToSend(Recipient recipient) {
		log.debug("Checking if document needs to be sent to recipient. recipient: " + recipient.getId());
		if (recipient.getSendCount() != null && recipient.getSendCount() != 0) {
			// if the document was already sent too many times, then ignore it
			if (recipient.getSendCount() >= maxResendCount) {
				log.debug("Document was already sent too many times");
				return false;
			}
			// if the document was sent less than configured as timeout time
			// ago, then resend it
			Long timePassed = (new Date()).getTime()
					- recipient.getLastSendDate().getTime();
			if (timePassed <= timeoutToResend*1000) {
				log.debug("Document was already sent recently.");
				return false;
			}
		}
		log.debug("Document need to be sent.");
		return true;
	}*/
			...
			if (recipient.getSendStatusID() == 101
				...
				try {
                    ...
                    if(asutus.getRegistrikood2() != null) {
	                    DecContainer container =
	                            (DecContainer) pkg.getParsedContainer();
	                    for(DecRecipient decRecipient : container.getTransport().getDecRecipient()) {
                   ...
                   /* OutgoingDhxPackage pkg = packageProvider.getOutgoingPackage(new File(pipelineDataFile),
					...
					DhxSendDocumentResult response = documentService
							.sendPackage(pkg);
					SendDocumentResponse docResponse = response.getResponse();
					if (docResponse.getFault() != null) {
						dvk.core.Fault fault = new Fault();
						fault.setFaultCode(docResponse.getFault()
								.getFaultCode());
						fault.setFaultString(docResponse.getFault()
								.getFaultString());
						recipient.setFault(fault);
					}
					recipient.setSendingEndDate(new Date());
					if (docResponse.getFault() == null) {
						log.debug("Document was succesfulyy sent to DHX");
	...
	public void saveSendResult(DhxSendDocumentResult finalResult,
		...
		try {
			...
			if (docResponse.getFault() == null) {
				...
				fault.setFaultDetail(faultString.substring(0, (faultString.length()>1900?1900:faultString.length())));
				recipient.setFault(fault);
			}	
			recipient.update(conn, null);
		}
